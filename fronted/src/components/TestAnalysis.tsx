...
const TestAnalysis: React.FC<TestAnalysisProps> = ({
  ...
  const totalTime = Object.values(result.questionTimes).reduce(
    (sum, time) => sum + time,
    0
  );
  ...
  // Move saveTestHistory inside useEffect
  useEffect(() => {
    const saveTestHistory = async () => {
      try {
        await fetch("/api/save-test-history", {
          method: "POST",
          headers: {
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-lg shadow-sm">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Taken on: {result.testDate.toLocaleString()}
            </p>
          </div>
                  ...
                  <div key={section} className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="grid grid-cols-5 gap-2">
                      {Array.from({ length: 25 }, (_, i) => {
                        ...
                        const isCorrect =
                          answer === result.correctAnswers?.[questionNum];
                        ...
                        return (
                          ...
                          `}
                            title={`Time taken: ${formatTime(
                              result.questionTimes[questionNum] || 0
                            )}`}
                          >
                            {questionNum}
                          </div>
